...
<div id="oosoft-2fa-section">
	<h2><?php esc_html_e( 'Two-Factor Authentication (2FA)', 'oosoft-2fa' ); ?></h2>

	<?php if ( $is_forced ) : ?>
		<div class="notice notice-warning inline">
			<p><?php esc_html_e( 'Your role requires 2FA. You must configure at least one method.', 'oosoft-2fa' ); ?></p>
		</div>
	<?php endif; ?>
	...
	<table class="form-table" role="presentation">
		...
		<?php if ( ! $is_forced ) : ?>
		<tr>
			<th scope="row"><?php esc_html_e( 'Enable 2FA', 'oosoft-2fa' ); ?></th>
			<td>
				<label>
					<input type="checkbox" name="oosoft_2fa_opted_in" value="1" <?php checked( $opted_in ); ?>>
					<?php esc_html_e( 'Enable two-factor authentication for my account', 'oosoft-2fa' ); ?>
				</label>
			</td>
		</tr>
		<?php endif; ?>
		...
		<tr>
			<th scope="row"><?php esc_html_e( 'Authenticator App (TOTP)', 'oosoft-2fa' ); ?></th>
			<td>
				<?php if ( $totp_enabled ) : ?>
					<p>
						<span class="oosoft-2fa-badge oosoft-2fa-badge--ok">&#10003; <?php esc_html_e( 'Configured', 'oosoft-2fa' ); ?></span>
					</p>
					<button type="button" id="oosoft-2fa-disable-totp" class="button button-secondary">
						<?php esc_html_e( 'Remove authenticator app', 'oosoft-2fa' ); ?>
					</button>
				<?php else : ?>
					<p class="description"><?php esc_html_e( 'Use Google Authenticator, Authy, or any TOTP-compatible app.', 'oosoft-2fa' ); ?></p>
					<button type="button" id="oosoft-2fa-start-totp" class="button button-primary">
						<?php esc_html_e( 'Set up authenticator app', 'oosoft-2fa' ); ?>
					</button>
					...
					<div id="oosoft-2fa-totp-wizard" style="display:none;margin-top:15px">
						<ol>
							<li>
								<p><?php esc_html_e( 'Scan this QR code with your authenticator app:', 'oosoft-2fa' ); ?></p>
								<div id="oosoft-2fa-qr-container" style="margin:10px 0">
									<div id="oosoft-2fa-qr-code"></div>
								</div>
								<p>
									<?php esc_html_e( 'Or enter this secret key manually:', 'oosoft-2fa' ); ?><br>
									...
									<button type="button" id="oosoft-2fa-copy-secret" class="button-link"
									        aria-label="<?php esc_attr_e( 'Copy secret key', 'oosoft-2fa' ); ?>">
										<?php esc_html_e( 'Copy', 'oosoft-2fa' ); ?>
									</button>
								</p>
							</li>
							<li>
								<p><?php esc_html_e( 'Enter the 6-digit code from your app to confirm setup:', 'oosoft-2fa' ); ?></p>
								...
								<button type="button" id="oosoft-2fa-confirm-totp" class="button button-primary">
									<?php esc_html_e( 'Verify and enable', 'oosoft-2fa' ); ?>
								</button>
		...
		<tr>
			<th scope="row"><?php esc_html_e( 'Email OTP', 'oosoft-2fa' ); ?></th>
			<td>
				<span class="oosoft-2fa-badge oosoft-2fa-badge--ok">&#10003; <?php esc_html_e( 'Available', 'oosoft-2fa' ); ?></span>
				<p class="description">
					<?php
					printf(
						/* translators: %s: email address */
						esc_html__( 'A one-time code will be sent to %s when you log in.', 'oosoft-2fa' ),
		...
		<!-- Backup codes -->
		<tr>
			<th scope="row"><?php esc_html_e( 'Backup Codes', 'oosoft-2fa' ); ?></th>
			<td>
				<?php if ( $backup_count > 0 ) : ?>
					<span class="oosoft-2fa-badge oosoft-2fa-badge--ok">
						...
						printf(
							...
							esc_html( _n( '%d code remaining', '%d codes remaining', $backup_count, 'oosoft-2fa' ) ),
							(int) $backup_count
						);
						?>
					</span>
				<?php else : ?>
					<span class="oosoft-2fa-badge oosoft-2fa-badge--warn">
						<?php esc_html_e( 'No backup codes', 'oosoft-2fa' ); ?>
					</span>
				<?php endif; ?>

				<p class="description"><?php esc_html_e( 'Backup codes let you access your account if you lose access to your authenticator app.', 'oosoft-2fa' ); ?></p>

				<button type="button" id="oosoft-2fa-gen-backup" class="button button-secondary">
					<?php $backup_count > 0 ? esc_html_e( 'Regenerate backup codes', 'oosoft-2fa' ) : esc_html_e( 'Generate backup codes', 'oosoft-2fa' ); ?>
				</button>

				<div id="oosoft-2fa-backup-codes-display" style="display:none;margin-top:15px">
					<div class="notice notice-warning inline">
						<p><strong><?php esc_html_e( 'Save these codes now!', 'oosoft-2fa' ); ?></strong>
						<?php esc_html_e( 'They will not be shown again. Each code can be used only once.', 'oosoft-2fa' ); ?></p>
					</div>
